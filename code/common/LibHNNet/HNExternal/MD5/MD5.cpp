...
std::string MD5_CTX::MD5String(const std::string& str)
{
	unsigned char szMD5Buffer[16] = {0};
	MD5_CTX Md5;
	Md5.MD5Update((unsigned char *)str.c_str(), str.size());
	Md5.MD5Final(szMD5Buffer);

	char szMD5[50] = {0};
	for (int i = 0; i < 16; i++)
	{
		sprintf(&szMD5[i*2], "%02x", szMD5Buffer[i]);
	}
	return szMD5;
}
