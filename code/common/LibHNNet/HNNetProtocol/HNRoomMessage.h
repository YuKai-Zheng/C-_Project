...
#define	ASS_GR_CONNECT_SUCCESS			3						//连接成功				
...
#define MDM_GR_QUEUE_MSG				114						//排队消息(无用)
...
#define ASS_GR_DISSOLVE_VIPROOM			27						//解散VIP房间
#define ASS_GR_VIPROOM_BASEINFOASK		28						//VIP房间基础信息请求
...
#define	ASS_GR_CONTEST_RECORD			25					//（无用）个人参赛纪录，登陆完成后服务端主动发送NET_ROOM_CONTEST_RECORD_RESULT
...
#define ASS_NOTIFYVIPUSER_WINLOSE		153						///通知内部玩家输赢情况
#define ASS_NOTIFYVIPUSER_GAMELOG		154						///发送玩家对局状况到客户端
/*********************************************************************************/
///错误代码
...
}GNUC_PACKED MSG_GR_SR_MatchDelete;
...
//解散VIP房间请求
struct MSG_GR_S_DissolveVipRoom_Ask
{
    BYTE								bDeskIndex;							//桌子索引
};

//解散VIP房间回复
struct MSG_GR_S_DissolveVipRoom_Req
{
    int									nResult;							//结果	0-成功 1-不是房主，不能解散房间
};

//VIP房间玩家对局信息
struct VipPlayerGameLog
{
	int				nUserID;				//玩家编号
	int             nGameCount[3];          //最近3局的游戏第几局
	LLONG			i64EveryGame[3];		//最近3局的输赢
	LLONG			i64SumWinLoseMoney;		//总的输赢金币
};
//发送玩家对局状况到客户端
struct VipRoomPlayerGameLogMsg
{
	VipPlayerGameLog	mUserLog[6];
	VipRoomPlayerGameLogMsg()
	{
		memset(&mUserLog, 0, sizeof(mUserLog));
	}
};
...
#define MDM_GR_MONEY					107						//银行消息
#define ASS_GR_CHECK_OUT				2						//取款		
#define	ASS_GR_CHECK_IN					3						//存款

//取钱
typedef struct tagMSG_GR_Check_money
{
	int     dwUserID;           //玩家ＩＤ
	LLONG	i64CheckOut;		//取出多少钱
	LLONG	i64MoneyInBank;		//没有使用
	LLONG   i64MoneyInRoom;		//没有使用
	int		uGameID;            //游戏ID
}GNUC_PACKED MSG_GR_Check_money;

//存钱
typedef struct tagMSG_GR_CheckIN_money
{
	int     dwUserID;		//玩家ＩＤ
	LLONG	i64CheckIN;		//存多少钱
	LLONG	i64MoneyInBank; //没有使用
	LLONG   i64MoneyInRoom; //没有使用
	int		uGameID;        //游戏ID
}GNUC_PACKED MSG_GR_CheckIN_money;
//通知内部玩家输赢情况
struct VipUserWinLoseRlt
{
    int				nWinLose;			//输赢情况 0-本局是输 1-本局是赢
    VipUserWinLoseRlt()
    {
        nWinLose = 0;
    }
...
} GNUC_PACKED MSG_GR_I_ContestInfo;
...
} GNUC_PACKED MSG_GP_GetContestRoomID;
...
} GNUC_PACKED DL_GetContestRoomID;
...
} GNUC_PACKED MSG_GP_GETMatchBeginTime;
...
} GNUC_PACKED MSG_GR_ContestChange;
...
typedef struct tagMSG_GR_ContestAward
{
	...
	int									iAward;
	int									iAwardType;
} GNUC_PACKED MSG_GR_ContestAward;
