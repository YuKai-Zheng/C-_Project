...
//支付宝网页支付地址
static std::string alipayWebURL;
//不同地区支付类型
...
//是否开启更新
static bool ONLY_186_TRY;

//微信登录相关信息
static std::string WX_APPID;
static std::string WX_SECRET_KEY;
static std::string WX_IMG_URL;
...
// 平台的登录ID地址
extern void setPlatformServerAddress(std::string url);
std::string getPlatformServerAddress();
std::string getPlatformServerHost();

// 获取API接口地址
extern void setAPIServerUrl(std::string url);
std::string getAPIServerUrl();

// 获取域名Url
extern void setWebServerUrl(std::string url);
std::string getWebServerUrl();

extern void setUpdateURL(std::string url);
