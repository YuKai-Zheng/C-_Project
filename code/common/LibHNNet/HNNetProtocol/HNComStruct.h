...
typedef struct tagComRoomInfo 
{
   ...
   /**************************1.1版本新增字段**************************************/	
	...
	//2016 7 18 wzx 添加房间等级成员变量
	//UINT							uLevelID;									//房间(初级场...)对应的ID long:2016/7/18
	//UINT							uMinBaseMoney;						//房间基础最小金币（比最少金币还少）long:2016/7/18
	//LLONG						i64Jackpot;								//房间奖池long:2016/7/18
...
typedef struct tagUserInfoStruct 
{
	...
	INT							dwAccID;							//ACC 号码
	INT							dwPoint;							//分数
	LLONG						i64Money;							//原来金币逻辑
	//  玩家信息中添加新字段  added by HBC
	//LLONG						i64Score;							//积分
	//LLONG						i64Coin;							//金币
...
///获取游戏公告列表回复
struct MSG_GetNoticeListInfo
{
    char		szNoticeStr[200];					//公告内容信息
    
    MSG_GetNoticeListInfo()
    {
        memset(szNoticeStr, 0, sizeof(MSG_GetNoticeListInfo));
    }
};
