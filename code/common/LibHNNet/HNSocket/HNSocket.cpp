...
int HNSocket::tcp_connect(const CHAR* ip, INT port) {
    INT nready = -1;
    ...
    for (aip = ailist; aip != NULL; aip = aip->ai_next) {
        ...
        _socket = socket(aip->ai_family, SOCK_STREAM, 0);
        if ((sockfd = _socket) < 0) {
        ...
        nready = ::connect(sockfd, aip->ai_addr, aip->ai_addrlen);
        if (nready < 0) {
...
//        //HNLOG("%s%d",m_aip->ai_addr->sa_data, m_aip->ai_addrlen);
//        nready = ::connect(_socket, m_aip->ai_addr, m_aip->ai_addrlen);
//        //if (nready< 0) {
//        //}
//    }
//    return nready;
