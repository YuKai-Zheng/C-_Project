...
bool GetMacAddress(char *pMacBuffer) {
  ...
  sprintf(pMacBuffer, "%02x:%02x:%02x:%02x:%02x:%02x", *ptr, *(ptr + 1),
          *(ptr + 2), *(ptr + 3), *(ptr + 4), *(ptr + 5));
  free(buf);
  return true;
}
...
+ (NSString *)getOrientation {
  ...
  NSString *OrientationValue =
      [infoDict objectForKey:@"UIInterfaceOrientation"];
...
+ (NSString *)getGameID;
{
  ...
  NSAssert(gameID != nil, @"In the info.plist file, the koogame_config "
                          @"configuration gameid is null");
  if (gameID == nil) {
    return @"";
  }
  return [gameID
...
+ (NSString *)getChannelID;
{
  ...
  NSAssert(channelID != nil, @"In the info.plist file, the koogame_config "
                             @"configuration channel_id is null");
  if (channelID == nil) {
    return @"";
  }
...
+ (NSString *)getAppID {
  ...
  NSAssert(appid != nil, @"In the info.plist file, the koogame_config "
                         @"configuration appid is null");
  if (appid == nil) {
    return @"";
  }
  return [appid
...
+ (void)dealPhone:(NSString *)number {
  ...
  NSString *telNumber = [NSString stringWithFormat:@"tel://%@", number];
  [callView loadRequest:[NSURLRequest
                            requestWithURL:[NSURL URLWithString:telNumber]]];
...
+ (NSString *)GetNetWorkType {
  ...
  //  NSLog(@"GetNetWorkType() strNetworkType :  %s", strNetworkType);
