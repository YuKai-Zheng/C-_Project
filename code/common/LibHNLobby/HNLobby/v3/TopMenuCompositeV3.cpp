...
#define btn_back_tag "btn_back"
#define btn_set_tag "btn_set"

using namespace HN;

static char* CAISHEN_CSB = "common/animation/Newyear/Icon/caishen.csb";

TopMenuCompositeV3::TopMenuCompositeV3() :
    _isTouch(true), _orignalChargePosition(Vec2::ZERO), _orignalBankPosition(Vec2::ZERO) {
    //_backsTageOpen.bank = false;
    //_backsTageOpen.recharge = false;
    //_backsTageOpen.rankList = false;

}

bool TopMenuCompositeV3::init() {
    if (!Node::init()) {
        return false;
    }

    _spreadLogic.init(this);
    _lobbyTop.node = CSLoader::createNode(PLATFORM_TOP_MENU_V3);
    //_lobbyTop.node->setPosition(Vec2(WIN_SIZE.width / 2, WIN_SIZE.height));
    addChild(_lobbyTop.node, PLATFORM_UI_ZORDER_TOP);

    _lobbyTop.layout = static_cast<Layout *>(_lobbyTop.node->getChildByName("Panel_top"));
    _lobbyTop.layout->setPosition(Vec2(0, 0));

    _lobbyTop.Sprite_topTitle = Sprite::create("v3/bg_up.png");
    auto titleSize = _lobbyTop.Sprite_topTitle->getContentSize();
    _lobbyTop.Sprite_topTitle->setPosition(Vec2(_lobbyTop.Sprite_topTitle->getContentSize().width * 0.5f, titleSize.height * 0.22));
    ...
    _lobbyTop.Button_Exit->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));


    _lobbyTop.Button_quicklyGame = (Button *)Helper::seekWidgetByName(_lobbyTop.layout, "Button_quickly");
    _lobbyTop.Button_quicklyGame->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));
    ...
    _lobbyTop.Button_Return->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));


    //保险箱
    _lobbyTop.Button_Bank = (Button *)Helper::seekWidgetByName(_lobbyTop.layout, "Button_bank");
    _lobbyTop.Button_Bank->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));
    ...
    _lobbyTop.Button_Charge->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));
	...
	_lobbyTop.Button_Charge->setVisible(PlatformConfig::getInstance()->funcConfig.isOpenRechargeFunc);
    ...
    _lobbyTop.Button_Setting->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));

    // 新春活动
    _lobbyTop.Button_newYear = (Button *)Helper::seekWidgetByName(_lobbyTop.layout, "Button_newYear");
    _lobbyTop.Button_newYear->setShowEffect(false);
    auto newYearX = _lobbyTop.Button_newYear->getPositionX();
    _lobbyTop.Button_newYear->setPositionX(newYearX - 15);
    _lobbyTop.Button_newYear->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));
	_lobbyTop.Button_newYear->setVisible(PlatformConfig::getInstance()->funcConfig.isOpenYearFunc);

    auto diBgcsb = CSLoader::createNode(CAISHEN_CSB);
    diBgcsb->setPosition(Vec2(_lobbyTop.Button_newYear->getContentSize().width *0.5f, _lobbyTop.Button_newYear->getContentSize().height *0.5f));
    _lobbyTop.Button_newYear->addChild(diBgcsb);

    auto antAction = CSLoader::createTimeline(CAISHEN_CSB);
    _lobbyTop.Button_newYear->runAction(antAction);
    antAction->play("Clicktips", true);

    //排行榜
    _lobbyTop.Button_rankList = Button::create("v3/button_up/paihangbang.png", "v3/button_up/paihangbang.png", "");
    ...
    _lobbyTop.Button_rankList->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));
    _lobbyTop.Button_Charge->getParent()->addChild(_lobbyTop.Button_rankList);
	_lobbyTop.Button_rankList->setVisible(PlatformConfig::getInstance()->funcConfig.isOpenRankFunc);
    ...
    _lobbyTop.globButton->addTouchEventListener(CC_CALLBACK_2(TopMenuCompositeV3::menuLobbyTopCallBack, this));

    return true;
}

void TopMenuCompositeV3::moveOut(bool isOut) {
    int s = isOut ? 1 : -1;
    //int k = isOut ? -1 : 1;

    if (!isOut) {
        this->runAction(Sequence::create(MoveBy::create(0.3f, Vec2(0, this->getContentSize().height * s)), nullptr));
    } else {
        this->runAction(Sequence::create(MoveBy::create(0.3f, Vec2(0, this->getContentSize().height * s)), nullptr));
    }
}

void TopMenuCompositeV3::menuLobbyTopCallBack(Ref *pSender, Widget::TouchEventType type) {
    ...
    } else if (name.compare("Button_newYear")==0) {
        if (platformDelegate) {
...
void TopMenuCompositeV3::walletChanged(LLONG money) {
}

void TopMenuCompositeV3::checkGiftMessages() {
    CommandManager::getInstance()->execute(CHECK_GIFT_COM);
}

void TopMenuCompositeV3::controlChargeAndBankBtnPosition(bool openCharge, bool openBank, bool openrank, bool forceHide) {
    if (forceHide) {
        _lobbyTop.Button_Charge->setVisible(false && !getExtendVersion());
...
//根据后台控制开关 判断是否可以显示功能按钮
void TopMenuCompositeV3::canDisplayButton(Node *node, bool isDisplay) {
    node->setVisible(isDisplay);
}

void TopMenuCompositeV3::setLogoTexture(std::string file) {
    char str[100] = { 0 };
    sprintf(str, "%s%d.png", "platform/lobbyUi/PlatformUi_TOP/",
            GameCreator()->getCurrentGameNameID());

    //_lobbyTop.Sprite_LogoSpr->setTexture(str);
}

void TopMenuCompositeV3::setShowBank(bool isShow) {
    isShowBank = isShow;
}

void TopMenuCompositeV3::setBacksTageOpen(BacksTageOpen open) {
    _backsTageOpen = open;

    controlChargeAndBankBtnPosition(_backsTageOpen.recharge, _backsTageOpen.bank, _backsTageOpen.rankList);

    //_lobbyTop.Button_Bank->setVisible(_backsTageOpen.bank);
    //_lobbyTop.Button_Charge->setVisible(_backsTageOpen.recharge);
    //_lobbyTop.Button_rankList->setVisible(_backsTageOpen.rankList);
}

void TopMenuCompositeV3::getRoomSpriteByRoomName(char name[], char fishName[], char* gameName) {
    char str[100] = { 0 };
    char bStr[100] = { 0 };
...
void TopMenuCompositeV3::setGameDeskLayer(GameDesk* gameDesk) {
    _gameDesk = gameDesk;
}

void TopMenuCompositeV3::setRoomInfo(ComRoomInfo* roomInfo) {
    this->roomInfo = roomInfo;
}

void TopMenuCompositeV3::newYearActivityCallBack(Ref* pSender, Widget::TouchEventType type) {
    ...
    ////loadinglayer::createloading(this, gfontconfig_22,gbktoutf8("加载中..."), loading);

    //CommandManager::getInstance()->addLoaderCommand(GBKToUtf8("加载中..."));
}

void TopMenuCompositeV3::setType(MenuType type) {
    if (type == game_list_type || type == game_type_type) {
        ...
        canDisplayButton(_lobbyTop.Button_newYear, true);
...
void TopMenuCompositeV3::autoMove() {
    //int s = isOut ? 1 : -1;
