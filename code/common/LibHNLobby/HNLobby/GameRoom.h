...
private:
    ...
    INT												_roomID;
    char*												_gameName;
    char												_gameRoomChangci[100];

    vector<ComRoomInfo*>				_vecComRoomInfo;
...
public:
    ...
    //获取当前房间ID
    char* getChangCiStr() {
        return _gameRoomChangci;
    }
    // 功能接口
private:
    void loadRoom(INT roomCount, Point pos[]);
    void getRoomSpriteByRoomName(char name[] ,char bili[],char*);
    // 创建房间列表
    void createRoomList();
    // 创建房间
