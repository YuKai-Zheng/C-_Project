...
const char* SHOP_TITLE = "v66/module/shangcheng/0.png";

const char* SHOP_CSB_V66 = "csb/v66/shop/shopLayer.csb";
const char* SHOP_ITEM_V66 = "csb/v66/shop/shopItem.csb";
//const char* ITEM_SPRITE_MONEY_V66 = "v66/set_bank/shangcheng/zhanshipai/level%d/1.png";
//const char* ITEM_SPRITE_PRICE_V66 = "v66/set_bank/shangcheng/zhanshipai/level%d/2.png";
//const char* BTN_TITLE_ALIPAY_IMG = "v66/button/blue/4.png";
//const char* BTN_TITLE_WXPAY_IMG = "v66/button/green/7.png";
//const char* BTN_TITLE_IAP_IMG = "v66/button/green/iap.png";

GameShopV66::~GameShopV66() {
    PlatformLogic()->removeEventSelector(MDM_GP_NOTIFY_PAY, ASS_GP_NOTIFY_PAY);
}

bool GameShopV66::init() {
    if (!HNLayer::init()) {
        return false;
    }
	...
	closeBtn->setPosition(Vec2(bg->getContentSize().width, bg->getContentSize().height - 50));
    ...
    auto title = Sprite::create(SHOP_TITLE);
...
//	alipayBtn = AUIButton::create(BTN_BLUE_BG_V66, BTN_BLUE_BG_V66);
//	nodeCsb->addChild(alipayBtn);
//	alipayBtn->setName(Alipay_Tag);
//	alipayBtn->setTitle(BTN_TITLE_ALIPAY_IMG);
//	alipayBtn->setTitleOffset(Vec2(-4, 4));
//	alipayBtn->setPosition(btnAlipay->getPosition());
//	alipayBtn->addTouchEventListener(CC_CALLBACK_2(GameShopV66::handlePayBtnEvent, this));
//
//	auto btnWeChat = (Button*)nodeCsb->getChildByName("btn_wx");
//	btnWeChat->setVisible(false);
//	wechatBtn = AUIButton::create(BTN_GREEN_BG_V66, BTN_GREEN_BG_V66);
//	nodeCsb->addChild(wechatBtn);
//	wechatBtn->setName(Weixin_Pay_Tag);
//	wechatBtn->setTitle(BTN_TITLE_WXPAY_IMG);
//	wechatBtn->setTitleOffset(Vec2(-4, 4));
//	wechatBtn->setPosition(btnWeChat->getPosition());
//	wechatBtn->addTouchEventListener(CC_CALLBACK_2(GameShopV66::handlePayBtnEvent, this));
//    if (wechatBtn) {
...
//        }
//        
//        IAPBtn = AUIButton::create(BTN_GREEN_BG_V66, BTN_GREEN_BG_V66);
//        nodeCsb->addChild(IAPBtn);
//        IAPBtn->setName(IAP_Tag);
//        IAPBtn->setPosition(alipayBtn->getPosition());
//        IAPBtn->setTitle(BTN_TITLE_IAP_IMG);
//        IAPBtn->setTitleOffset(Vec2(-4, 4));
//        IAPBtn->addTouchEventListener(CC_CALLBACK_2(GameShopV66::handlePayBtnEvent, this));
//    }
//#endif
		...
		if (_product) {
			if (Tools::isiOSReview()) {
				if (_IAP_supported) {
					...
					CALLBACK_PRAGMA pragma(HN_PAY_CALLBACK(GameShopV66::payCB, this));
...
//#endif
	...
	for (int i = 0; i < nItemCount; i++)
	{
		...
		auto node = CSLoader::createNode(SHOP_ITEM_V66);
		auto item = dynamic_cast<Button*>(node->getChildByName("btn_item"));
        item->setTouchEnabled(false);
        
        sprintf(temp, "v66/module/shangcheng/icon/%d.png", i + 1);
...
void GameShopV66::selectItemCallback(Ref* pSender, Widget::TouchEventType type)
{
...
void GameShopV66::handleBuyBtnEvent(Ref* pSender, Widget::TouchEventType type) {
...
void GameShopV66::handleBackBtnEvent(Ref* pSender, Widget::TouchEventType type) {
    if (Widget::TouchEventType::ENDED != type)
        return;
...
void GameShopV66::handlePayBtnEvent(Ref* pSender, Widget::TouchEventType type) {
    if (Widget::TouchEventType::ENDED != type)
        return;
...
void GameShopV66::open(Node* parent, Vec2 vec, int zorder, int tag) {
...
void GameShopV66::close() {
    //runAction(Sequence::create(ScaleTo::create(0.2f, 0.2f), RemoveSelf::create(), nullptr));
	removeFromParent();
}

void GameShopV66::setMenuDelegate(MenuDelegate* delegate) {
    _delegate = delegate;
}

//请求支付
void GameShopV66::executePayment(PRODUCT_INFO* productInfo) {
...
std::string GameShopV66::addOrder_URL(const char* userName, DOUBLE price, std::string payTypeInfo) {
...
//支付回调
void GameShopV66::payCB(const std::string& args) {
    //bool ret = ProductManger::getInstance()->parsePayResult(args);

    HNLog::logInfo("----pay result:%s", args.c_str());
    std::string ext = "{'receipt-data':'xxxx','order-id':12345}";
...
void GameShopV66::payCallback_IAP(const std::string& data) {
    rapidjson::Document doc;
    ...
    request->setResponseCallback(CC_CALLBACK_2(GameShopV66::onHttpResponse, this));
...
void GameShopV66::onHttpResponse(network::HttpClient* client, network::HttpResponse* response) {
    std::vector<char> *buffer = response->getResponseData();
    std::string data;
    data.assign(buffer->begin(), buffer->end());
...
void GameShopV66::payOnWebWithBrower(DOUBLE price) {
    char link[128] = {0};
