...
#include "HNLobby/v88/GameAutographV88.h"
...
//CSB
static const char* SET_SHENMING_CSB =	"csb/v88/setLayer/shenMing_view.csb";
static const char* SET_HELP_CSB =		"csb/v88/setLayer/help_view.csb";
static const char* SET_MODIFY_CSB =		"csb/v88/setLayer/modifyPassWordNode.csb";
static const char* SET_BIND_CSB = "csb/v88/setLayer/bindPhoneNode.csb";
static const char* SET_SAFE_CSB = "csb/v88/setLayer/set_safeBind_v88.csb";


static const char* SET_TITLE = "v88/set_bank/set/set.png";

//button menu
static const char* SET_MENU_BUTTON_NORMAL_BG = "v88/set_bank/button/6.png";
static const char* SET_MENU_BUTTON_SELECT_BG = "v88/set_bank/button/5.png";

static const char* SET_MENU_BUTTON_TITLE0 = "v88/set_bank/button/set/12.png";
static const char* SET_MENU_BUTTON_TITLE1 = "v88/set_bank/button/set/10.png";
static const char* SET_MENU_BUTTON_TITLE2 = "v88/set_bank/button/set/8.png";
static const char* SET_MENU_BUTTON_TITLE3 = "v88/set_bank/button/set/7.png";
static const char* SET_MENU_BUTTON_TITLE4 = "v88/set_bank/button/set/11.png";
static const char* SET_MENU_BUTTON_TITLE5 = "v88/set_bank/button/set/9.png";
static const char* SET_MENU_BUTTON_TITLE6 = "v88/set_bank/button/set/anquan.png";

//title
static const char* SET_BUTTON_GETCODE_TITLE = "v88/button/green/5.png";
static const char* SET_BUTTON_BIND_TITLE = "v88/button/green/8.png";
static const char* SET_BUTTON_UNBIND_TITLE = "v88/button/green/14.png";
static const char* SET_BUTTON_MODIFY_TITLE = "v88/button/green/4.png";
static const char* SET_BUTTON_SAFE_TITLE = "v88/button/green/tijiao.png";

static const char* SET_HELP_ITEM_PATH = "v88/shenming/%s.png";

#define WINSIZE Director::getInstance()->getWinSize()


GameSetLayerV88::GameSetLayerV88() {
	m_isBind = false;
    _oldViewTag = 0;
...
bool GameSetLayerV88::init() {
    ...
    title->setPosition(Vec2(bg->getContentSize().width/2, bg->getContentSize().height - 43));
    bg->addChild(title);
...
void GameSetLayerV88::open(Node* parent, Vec2 vec, int zorder, int tag) {
...
// 初始化按钮控件
void GameSetLayerV88::loadButton() {
...
//	vMenuTitle.push_back(SET_MENU_BUTTON_TITLE6);
	...
	for (int i = 0; i < vMenuTitle.size(); i++)
	{
		...
		menu->addTouchEventListener(CC_CALLBACK_2(GameSetLayerV88::menuButtonCallBack, this));
...
void GameSetLayerV88::menuButtonCallBack(Ref* pSender, Widget::TouchEventType type) {
...
//点击效果
void GameSetLayerV88::touchEffect(UINT tag) {
...
void GameSetLayerV88::handleModifyBtn(Ref* pSender, Widget::TouchEventType type) {
	...
	do {
		...
		PlatformLogic()->addEventSelector(MDM_GP_USERINFO, ASS_GP_USERINFO_ACCEPT, HN_SOCKET_CALLBACK(GameSetLayerV88::modifyPasswordSelector, this));
		PlatformLogic()->addEventSelector(MDM_GP_USERINFO, ASS_GP_USERINFO_NOTACCEPT, HN_SOCKET_CALLBACK(GameSetLayerV88::modifyPasswordSelector, this));
	} while (0);
}

bool GameSetLayerV88::modifyPasswordSelector(HNSocketMessage* socketMessage) {
...
void GameSetLayerV88::handleGetBtn(Ref* pSender, Widget::TouchEventType type) {
	if (Widget::TouchEventType::ENDED != type)
		return;
	HNAudioEngine::getInstance()->playEffect(GAME_SOUND_BUTTON);
	...
	request->setResponseCallback(CC_CALLBACK_2(GameSetLayerV88::onHttpCodeResponse, this));

	request->setRequestData(str.c_str(), str.size());
	network::HttpClient::getInstance()->sendImmediate(request);
}

void GameSetLayerV88::updateBindInfo(bool isBind, bool isSuccess)
{
	vertifyCode->setText("");
...
void GameSetLayerV88::handleSendBtn(Ref* pSender, Widget::TouchEventType type) {
	if (Widget::TouchEventType::ENDED != type)
		return;
	...
	request->setUrl(URL.c_str());
	request->setRequestType(network::HttpRequest::Type::POST);
	request->setResponseCallback(CC_CALLBACK_2(GameSetLayerV88::onHttpBindResponse, this));
...
void GameSetLayerV88::onHttpCodeResponse(HttpClient* client, HttpResponse* response)
{
...
void GameSetLayerV88::onHttpBindResponse(network::HttpClient* client, network::HttpResponse* response) {
	std::vector<char> recv = *response->getResponseData();
	std::string data;
	data.assign(recv.begin(), recv.end());
...
void GameSetLayerV88::createXiuGaiView(Node* parent) {
	...
	_oldPwd->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
	...
	_newPwd->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
	...
	_surePwd->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
	...
	modifyBtn->addTouchEventListener(CC_CALLBACK_2(GameSetLayerV88::handleModifyBtn, this));
}

void GameSetLayerV88::createBoundView(Node* parent) {
	...
	_phonenum->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
	...
	_vertifyCode->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
	...
	auto getBtn = AUIButton::create(BTN_GREEN_BG_V88, BTN_GREEN_BG_V88);
	...
	getBtn->addTouchEventListener(CC_CALLBACK_2(GameSetLayerV88::handleGetBtn, this));

	auto tempSendBtn = (Button*)bindLayer->getChildByName("Button_sure");
	tempSendBtn->setVisible(false);
	sendBtn = AUIButton::create(BTN_GREEN_BG_V88, BTN_GREEN_BG_V88);	
	sendBtn->setPosition(tempSendBtn->getPosition());
	bindLayer->addChild(sendBtn);
	sendBtn->addTouchEventListener(CC_CALLBACK_2(GameSetLayerV88::handleSendBtn, this));

	updateBindInfo(LoginResult.iBindMobile, true);
}

void GameSetLayerV88::createMusicView(Node* parent) {
    _oldViewTag = creatMusicEffect_Btn;
    auto music = MusicEffectV88::create();
	music->setPositionX(music->getPositionX()-300);
    parent->addChild(music);
}


void  GameSetLayerV88::createHelpView(Node* parent) {
...
void GameSetLayerV88::createShenMingView(Node* parent) {
...
void GameSetLayerV88::createSafeView(Node* parent)
{
	_oldViewTag = creatShenMing_Btn;
	...
	tijiaoBtn = AUIButton::create(BTN_GREEN_BG_V88, BTN_GREEN_BG_V88);
	...
	tijiaoBtn->addTouchEventListener(CC_CALLBACK_2(GameSetLayerV88::handleSafeBtn, this));
...
void GameSetLayerV88::handleSafeBtn(Ref* pSender, Widget::TouchEventType type)
{
	if (Widget::TouchEventType::ENDED != type)
		return;
	...
	do {
		...
		data.UserID = id;
		PlatformLogic()->sendData(MDM_GP_IMEI_Bind, ASS_GP_IMEI_Bind, &data, sizeof(data), HN_SOCKET_CALLBACK(GameSetLayerV88::safeMessageCallBack, this));
	} while (0);
}

bool GameSetLayerV88::safeMessageCallBack(HNSocketMessage* socketMessage) {
