...
static const char* FIND_NODE_CSB = "csb/v88/loginUI/findPwNode_v88.csb";

FindPwCompositeV88::FindPwCompositeV88() {
...
bool FindPwCompositeV88::init() {
    ...
    _find_node.backBtn->addTouchEventListener(CC_CALLBACK_2(FindPwCompositeV88::closeClickCallback, this));
    ...
    phoneText->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
    ...
    codeText->setPlaceHolderColor(FONT_COLOR_ORANGE_V88);
    ...
    _find_node.sendCodeBtn->addTouchEventListener(CC_CALLBACK_2(FindPwCompositeV88::sendCodeCallback, this));

    //find按钮
    auto btn = (Button*)_find_node.node->getChildByName("Button_find");
    btn->setVisible(false);

    _find_node.findBtn = AUIButton::create("v88/button/blue2/6.png", "v88/button/blue2/6.png");
    _find_node.findBtn->setName("Button_sendCode");
    _find_node.findBtn->setTitle("v88/button/blue2/8.png");
    _find_node.findBtn->setTitleOffset(Vec2(-4, 4));
    _find_node.findBtn->setPosition(btn->getPosition());
    _find_node.findBtn->addTouchEventListener(CC_CALLBACK_2(FindPwCompositeV88::sendFindCallback, this));
...
void FindPwCompositeV88::initFindPwUI() {
    _find_node.accountEdit->setString("");
    _find_node.phoneEdit->setString("");
    _find_node.codeEdit->setString("");
}

void FindPwCompositeV88::closeClickCallback(Ref* pSender, Widget::TouchEventType type) {
...
//发送验证码
void FindPwCompositeV88::sendCodeCallback(Ref* pSender, Widget::TouchEventType type) {
    ...
    request->setResponseCallback(CC_CALLBACK_2(FindPwCompositeV88::onHttpYanZhengResponse, this));
...
//找回密码
void FindPwCompositeV88::sendFindCallback(Ref* pSender, Widget::TouchEventType type) {
    auto btn = (Button*)pSender;
    ...
    case cocos2d::ui::Widget::TouchEventType::ENDED: {
        ...
        request->setUrl(URL.c_str());
        request->setRequestType(network::HttpRequest::Type::POST);
        request->setResponseCallback(CC_CALLBACK_2(FindPwCompositeV88::onHttpFindResponse, this));
...
void FindPwCompositeV88::onHttpFindResponse(network::HttpClient* client, network::HttpResponse* response) {
...
void FindPwCompositeV88::onHttpYanZhengResponse(network::HttpClient* client, network::HttpResponse* response) {
    std::vector<char> recv = *response->getResponseData();
    std::string data;
    data.assign(recv.begin(), recv.end());
...
void FindPwCompositeV88::editBoxEditingDidEnd(ui::EditBox* editBox) {

}

void FindPwCompositeV88::editBoxTextChanged(ui::EditBox* editBox, const std::string& text) {
