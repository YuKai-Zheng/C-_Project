...
static const char* MODIFYPASSWORD_JSON	= "csb/platform/modifyPassWord/modifyPassWordNode.csb";
...
bool ModifyPasswordLayer::init()
{
	...
	_ui.modifyLoader->setPosition(Vec2(0,0));
	...
	//旧密码 
	auto oldPassword = (TextField*)panel->getChildByName("TextField_oldPass");
	oldPassword->setVisible(false);
	_ui.TextField_oldPassword = HNEditBox::createEditBox(oldPassword, this);
	_ui.TextField_oldPassword->setPasswordEnabled(false);
	//新密码
	auto NewPassword = (TextField*)panel->getChildByName("TextField_newPass");
	NewPassword->setVisible(false);
	_ui.TextField_NewPassword = HNEditBox::createEditBox(NewPassword, this);
	_ui.TextField_NewPassword->setPasswordEnabled(false);
	//确认密码
	auto SurePassword = (TextField*)panel->getChildByName("TextField_surePass");
	SurePassword->setVisible(false);
	_ui.TextField_SurePassword = HNEditBox::createEditBox(SurePassword, this);
	_ui.TextField_SurePassword->setPasswordEnabled(false);
...
void ModifyPasswordLayer::okEventCallBack(Ref* pSender, Widget::TouchEventType type)
{
	...
	do
	{
		...
		{
			GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("请输入旧密码。")); break;
		}
		if (newPassword.empty())
		{
			GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("请输入新密码。")); break;
		}

		if (surePassword.empty())
		{
			GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("请输入确认密码。")); break;
		}
		if (0 != tmp.compare(MD5oldPass))
		{
			GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("旧密码输入错误。")); break;
		}
		if (0 != newPassword.compare(surePassword))
		{
			GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("新密码和确认密码不相同。")); break;
		}
...
bool ModifyPasswordLayer::modifyPasswordSelector(HNSocketMessage* socketMessage)
{
	if (ASS_GP_USERINFO_ACCEPT == socketMessage->messageHead.bAssistantID)	// 服务端已接受
	{
		...
		userDefault->flush();
		_userPswd.clear();
		GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("密码修改成功。"));
	}
	else if (ASS_GP_USERINFO_NOTACCEPT == socketMessage->messageHead.bAssistantID)	// 服务端未能接受
	{
		GamePromptLayer::create()->showPrompt(ForceGBKToUtf8("密码修改失败。"));
	}
	else
	{
