...
void UICoin::Play()
{
	...
	Animate* ptr_animate_coin = Animate::create(ptr_animation_coin);
...
//	SimpleAudioEngine::getInstance()->playEffect(CCFileUtils::getInstance()->fullPathForFilename("music/goldFall.mp3").c_str());
    HN::HNAudioEngine::getInstance()->playEffect(FileUtils::getInstance()->fullPathForFilename("music/goldFall.mp3").c_str());
	...
	for (iter = m_list_coin.begin(); iter != m_list_coin.end(); iter++)
	{
		...
		ptr_icon->runAction(Sequence::create(ptr_action_0, ptr_action_2, ptr_action_3, ptr_action_4, NULL));
	}
}
