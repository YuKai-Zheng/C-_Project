...
void LoadingLayer::createDialog(const TTFConfig& ttfConfig, const std::string& text, const std::string& image) {
	...
	//auto jsonPath = "hallimg/loading/jiazai.json";
	//auto atlasPath =  "hallimg/loading/jiazai.atlas";
	//auto skeletonNode = spine::SkeletonAnimation::createWithJsonFile(jsonPath, atlasPath);
	//if (skeletonNode) {
	//	skeletonNode->setAnchorPoint(Vec2(0.5, 0.5));
	//	skeletonNode->setPosition(this->getContentSize()/2);
	//	skeletonNode->setAnimation(0, "jiazai", true);
...
bool LoadingLayer::init() {
    ...
    this->scheduleOnce(schedule_selector(LoadingLayer::remove), 6.0f);

    return true;
}

void LoadingLayer::remove(float dt) {
