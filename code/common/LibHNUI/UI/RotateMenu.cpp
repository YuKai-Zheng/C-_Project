...
bool RotateMenu::init() {
    ...
    this->setContentSize(Size(s.width + 600,s.height - 200));
...
void RotateMenu::updatePosition() {
    ...
    for (int i = 0; i < _items.size(); i++) {
        ...
        _items.at(i)->setScale(0.7 + 0.5*cos(i*_unitAngle + getAngle()));
    }
    return;
}
void RotateMenu::updatePositionWithAnimation() {
    ...
    for (int i = 0; i < _items.size(); i++) {
        ...
        //缩放比例  0.5~1
        auto scaleTo = ScaleTo::create(animationDuration, 0.7 + 0.5*cos(i*_unitAngle + getAngle()));
