...
//	this->setPositionY(pos.y + 20);
	...
	this->setPositionPercent(module->getPositionPercent());
	//this->setPositionPercentXEnabled(module->isPositionPercentXEnabled());
	//this->setPositionPercentYEnabled(module->isPositionPercentYEnabled());

    /*
    if (VERSION_NINETY_NINE) {
        this->setPlaceholderFontColor(Color3B(39, 10, 0));
    } else if (VERSION_EIGHTY_EIGHT) {
        this->setPlaceholderFontColor(Color3B(125, 125, 125));
    } else
     */
    {
    ...
    this->setContentSize(Size(editSize.width, editSize.height));
    //设置键盘中return键显示的字符
