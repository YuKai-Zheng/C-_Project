...
namespace JPM {
    ...
    const static char* NL_BGM = "v66/music/NL/NL_BGM.mp3";
    const static char* NL_bingo = "v66/music/NL/NL_bingo.mp3";
    const static char* NL_startLoop = "v66/music/NL/NL_startLoop.mp3";
    const static char* NL_endLoop = "v66/music/NL/endLoop.wav";
    const static char* kIconTypeTpl = "Games/JPM/type/LT_%02d.png";
	...
	void GameTableUIV88::createRootLayer() {
        //background image
        ImageView* bg = ImageView::create("Games/JPM/ui/18.png");
        if (bg) {
            _root.bg = bg;
...
//        //pool
//        ImageView* poolBg = ImageView::create("v88/JPMs/ui/pool.png");
//        if (poolBg) {
//            _root.poolBg = poolBg;
...
//        }
        //middle Layer
        Node* middle = CSLoader::createNode("Games/JPM/Node_root.csb");
        if (middle) {
            ...
            if (_root.middlePanel) {
                ...
                auto leftNode = dynamic_cast<Node*>(_root.middlePanel->getChildByName("Node_left"));
                if (leftNode) {
                    auto leftfire = CSLoader::createNode("Games/JPM/Node_effect.csb");
                    auto leftaction = CSLoader::createTimeline("Games/JPM/Node_effect.csb");
                    if (leftfire && leftaction)
                    {
                        leftNode->addChild(leftfire);
                        leftfire->setPosition(Vec2::ZERO);
                        leftfire->runAction(leftaction);
                        leftaction->gotoFrameAndPlay(0);
                    }
                    
                }
                
                auto rightNode = dynamic_cast<Node*>(_root.middlePanel->getChildByName("Node_right"));
                if (rightNode) {
                    auto rightfire = CSLoader::createNode("Games/JPM/Node_effect.csb");
                    auto rightaction = CSLoader::createTimeline("Games/JPM/Node_effect.csb");
                    if (rightfire && rightaction)
                    {
                        rightNode->addChild(rightfire);
                        rightfire->setPosition(Vec2::ZERO);
                        rightfire->runAction(rightaction);
                        rightaction->gotoFrameAndPlay(0);
                    }
                    
                }
                
                auto jsonPath      = "Games/JPM/animation/deng/jx_deng.json";
                auto atlasPath     = "Games/JPM/animation/deng/jx_deng.atlas";
                ...
                m_RateBg = ImageView::create("Games/JPM/ui/sgdk.png");
                if (m_RateBg) {
    ...
    void GameTableUIV88::createGameSettlementLayer() {
       /* _gameSettlement = nullptr;
        JPMSettlementV88* settle = JPMSettlementV88::create();
        if (settle) {
...
//            HNLog::logInfo("update line effect End: %d %d", m_nHasTurnUnitCount);
    ...
    void GameTableUIV88::checkHasReward(long long _winmoney)
    {
        ...
        std::string pathMusic = "v66/music/NL/";
        if (_realWinMoney > 0) {
