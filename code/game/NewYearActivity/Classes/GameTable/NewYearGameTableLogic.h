#ifndef __LEGameTableLogic_h__
#define __LEGameTableLogic_h__
...
namespace NewYearActivity
{
    ...
    public:
		...
		//活动请求数据
		void sendActivityType(int nType);

		//签到
		void sendSign();
		//赶年兽
		void sendDriveNianShou(int nUserID, int nType);
		//请求轮盘结果
		void sendTurnTableResult(int nUserID);
		//请求是否进行过游戏
		void sendIsPlay(int userid);
    };
