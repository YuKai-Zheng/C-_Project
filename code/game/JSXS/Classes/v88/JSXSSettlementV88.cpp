...
namespace JSXS {
	...
	JSXSSettlementV88::JSXSSettlementV88() {
    }
    
	JSXSSettlementV88::~JSXSSettlementV88() {
    }
    
	bool JSXSSettlementV88::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void JSXSSettlementV88::showSettleLayerAndUpdateData(struct_s2c_game_begin msg) {
        int count=0;
        m_totalRate = 0;
        scoreCellVec.clear();
        for (int i = 0; i < LINE_COUNT; i++) {
            if (msg.nOpenLineLotteryType[i] != -1) {
                JSXSScorePanelCellV88* newCell = JSXSScorePanelCellV88::create();
    ...
    void JSXSSettlementV88::hideSettleLayer() {
    ...
    void JSXSSettlementV88::initSettleLayer() {
        m_audioEffectID = 0;
        m_root = nullptr;
        m_action = nullptr;
        scoreCellVec.clear();
