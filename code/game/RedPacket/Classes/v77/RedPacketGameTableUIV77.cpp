...
#include "RedPacketGameTableUIV77.h"
...
#include "RedPacketOpenDetailsLayerV77.h"
...
HNGameUIBase* GameTableUIV77::create(BYTE bDeskIndex, bool bAutoCreate) {
    GameTableUIV77* tableUI = new GameTableUIV77();
...
GameTableUIV77::~GameTableUIV77() {
//    this->unschedule(schedule_selector(GameTableUIV77::updateClock));
//    this->unschedule(schedule_selector(GameTableUIV77::updateTip));
//    this->unschedule(schedule_selector(GameTableUIV77::updateLoopSpritePosition));
//    this->unschedule(schedule_selector(GameTableUIV77::updateResultAnimation));
//    this->unschedule(schedule_selector(GameTableUIV77::showGameStateLabelAnimation));
//    this->unschedule(schedule_selector(GameTableUIV77::updateRandomBlinkLightFree));

//    this->unschedule(schedule_selector(GameTableUIV77::autoRemoveNianShouButton));

    //this->unschedule(schedule_selector(GameTableUIV77::sendCardTimer));
//    this->unschedule(schedule_selector(GameTableUIV77::showOpenBtn));
    CC_SAFE_DELETE(_GameTableLogic);

    resetData();
}

bool GameTableUIV77::init(BYTE bDeskIndex, bool bAutoCreate) {
    ...
    myListener->onTouchBegan=CC_CALLBACK_2(GameTableUIV77::onTouchBegan,this);
...
//    this->setRotation(-90);
    ...
    void GameTableUIV77::createToolBarUI() {
    //    char resStr[128] = {0};
    //
    ...
    //    setBtn->addTargetWithActionForControlEvents(this, cccontrol_selector(GameTableUIV77::onBtnClick), Control::EventType::TOUCH_UP_INSIDE);
    //    _toolbarBg->addChild(setBtn);
        ...
        //helpBtn->addTargetWithActionForControlEvents(this, cccontrol_selector(GameTableUIV77::onBtnClick), Control::EventType::TOUCH_UP_INSIDE);
    ...
    //    leaveBtn->addTargetWithActionForControlEvents(this, cccontrol_selector(GameTableUIV77::onBtnClick), Control::EventType::TOUCH_UP_INSIDE);
    ...
    //    nextRoundOutBtn->addTargetWithActionForControlEvents(this, cccontrol_selector(GameTableUIV77::onBtnClick), Control::EventType::TOUCH_UP_INSIDE);
    ...
    void GameTableUIV77::createPacketsUI() {
        ...
        if (m_toolBtn) {
            ...
            m_toolBtn->addTouchEventListener(CC_CALLBACK_2(GameTableUIV77::toolBarButtonCallBack, this));
            addChild(m_toolBtn);
        }
        ...
        if (m_allPlayersBtn) {
            ...
            m_allPlayersBtn->addTouchEventListener(CC_CALLBACK_2(GameTableUIV77::allPlayersButtonCallBack, this));
        ...
        if (m_sendPacketBtn) {
            ...
            m_sendPacketBtn->addTouchEventListener(CC_CALLBACK_2(GameTableUIV77::sendPacketButtonCallBack, this));
...
//            m_myPacketHistoryBtn->addTouchEventListener(CC_CALLBACK_2(GameTableUIV77::myPacketHistoryButtonCallBack, this));
...
//            for (int i = 0; i < 55; i++) {
//                auto player = RedPacketGamePlayerCellLayerV77::create();
//                if (player) {
...
//        }
    ...
    void GameTableUIV77::leaveTheGame() {
#if (CC_TARGET_PLATFORM == CC_PLATFORM_IOS)
    ...
    //新红包通知
    void GameTableUIV77::onNotifyNewPacketResponse(RedPacket::tag_s2c_RedPacketListReq *packetRes) {
    ...
    void GameTableUIV77::onSendPacketResponse(RedPacket::tag_c2s_EmitRedPacketReq *packetRes) {
        if (packetRes) {
    ...
    void GameTableUIV77::onOpenPacketResponse(RedPacket::tag_c2s_RobRedPacketReq *packetRes) {
        if (packetRes) {
            //自己拆红包的数据存到对应的红包类里面
            if (m_packetCells) {
                RedPacketGamePlayerCellLayerV77* cell = (RedPacketGamePlayerCellLayerV77*)m_packetCells->getChildByName(to_string(packetRes->nRedPacketID));
                if (cell) {
                    cell->setMyPacketRobInfo(packetRes);
                }
            }
            //
            auto detailLayer = RedPacketOpenDetailsLayerV77::create();
            if (detailLayer) {
                ...
                detailLayer->setMyHistoryBtnCallBack([=]() {
                    ...
                    auto myHistory = RedPacketMyRecordLayerV77::create();
                    if (myHistory) {
...
//    void GameTableUIV77::onNotifyPlayerOpenPacketResponse(RedPacket::RobRedPacketUserInfo *packetRes) {
//        if (packetRes) {
...
//    }
    ...
    //红包详情基础信息通知更新到UI
    void GameTableUIV77::onNotifyPacketOpenDetailsResponse(RedPacket::tag_c2s_RedPacketRobLogReq *packetRes) {
        if (packetRes) {
            RedPacketOpenDetailsLayerV77* detailLayer = (RedPacketOpenDetailsLayerV77* )this->getChildByName("Open_Detail");
...
//            }
    ...
    void GameTableUIV77::onNotifyPacketOpenPlayerDetailResponse(RobRedPacketUserInfo* packetRes) {
        if (packetRes) {
            RedPacketOpenDetailsLayerV77* detailLayer = (RedPacketOpenDetailsLayerV77* )this->getChildByName("Open_Detail");
            if (detailLayer) {
...
//            }
    ...
    void GameTableUIV77::onGetMyPacketHistiryResponse(RedPacket::tag_s2c_MyHistoryRobLogReq *packetRes) {
        if (packetRes) {
            RedPacketMyRecordLayerV77* historyLayer = (RedPacketMyRecordLayerV77*)this->getChildByName("My_History");
            if (historyLayer) {
                auto cell = RedPacketMyRecordCellLayerV77::create();
                if (cell) {
    ...
    //更新所有玩家列表
    void GameTableUIV77::onUpdateAllPlayerListResponse(std::vector<tag_s2c_PlayerListMsgReq> allPlayer) {
        RedPacketAllPlayersListLayerV77* allLayer = (RedPacketAllPlayersListLayerV77*)this->getChildByName("All_Player_List");
    ...
    //红包领取完或者红包过期
    void GameTableUIV77::onNotityPacketExpiredMessage(RedPacket::tag_s2c_RobRedPacketOverReq *packetRes) {
        if (packetRes) {
    ...
    //更新身上的金币
    void GameTableUIV77::onUpdateMyPacketMoney(LLONG money) {
        if (m_packetCells && m_lastRequestMoneyByPacket > 0) {
            RedPacketGamePlayerCellLayerV77* cell = (RedPacketGamePlayerCellLayerV77*)m_packetCells->getChildByName(to_string(m_lastRequestMoneyByPacket));
            if (cell) {
    ...
    //更新房间基础信息展示
    void GameTableUIV77::onUpdateRoomBaseInfo() {
    ...
    //////////////////////////////////////////////////////////////////////////
    void GameTableUIV77::onBtnClick(Ref* pSender,Control::EventType event) {
        
    }

    bool GameTableUIV77::onTouchBegan(Touch* touch, Event* event) {
    ...
    void GameTableUIV77::onEnter() {
        HNGameUIBase::onEnter();
    }

    void GameTableUIV77::onExit() {
        HNGameUIBase::onExit();
    }

    void GameTableUIV77::onExitTransitionDidStart() {
        HNGameUIBase::onExitTransitionDidStart();
        HNAudioEngine::getInstance()->stopBackgroundMusic();
    }

    void GameTableUIV77::addUser(BYTE seatNo, bool bMe) {
       
    }


    void GameTableUIV77::setBtnTouch(Button* btn, bool bTouch) {
    ...
    void GameTableUIV77::tipCallback() {
//        if (tipMsg.bIsExit == 1) {
//            //准备关闭掉
...
//                unschedule(schedule_selector(GameTableUIV77::updateTip));
...
//            schedule(schedule_selector(GameTableUIV77::updateTip), 1.0f);
//        } else {
//
//        }
    }

    void GameTableUIV77::showNotice(const std::string& message, int nCount, int nType, bool bVisible) {
        if (bVisible) {
            if (!m_Notice) {
    ...
    //玩家坐下(非自己)
    void GameTableUIV77::userSitDown(MSG_GR_R_UserSit * userSit, UserInfoStruct* user) {
        addUser(userSit->bDeskStation, userSit->bDeskStation == _GameTableLogic->getMySeatNo());
    }

    //玩家站起
    void GameTableUIV77::userUp(MSG_GR_R_UserSit * userSit, UserInfoStruct* user) {
        
    }
        

    void GameTableUIV77::onGameTipExit(tag_s2c_tip_exit* msg) {
...
//        })));
    ...
    void GameTableUIV77::onHandleNewMessage(int iType, const std::string &message, int iCount)
    {
        //全服公告
        if (iType == 0)
        {
    ...
    void GameTableUIV77::resetNewGame() {
       
    }

    void GameTableUIV77::resetUINewGame() {

    }

    void GameTableUIV77::resetData() {
        //ui
        _fScaleX = 0;
        _fScaleY = 0;
        _tableSize = Size(0, 0);
...
//        m_myPacketHistoryBtn = nullptr;
    ...
    void GameTableUIV77::toolBarButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
    ...
    void GameTableUIV77::allPlayersButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        if (type != Widget::TouchEventType::ENDED) {
            return;
        }
        if (_GameTableLogic) {
        ...
        auto allPlayers = RedPacketAllPlayersListLayerV77::create();
        if (allPlayers) {
...
//            allPlayers->updateAllPlayers();
    ...
    void GameTableUIV77::sendPacketButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        if (type != Widget::TouchEventType::ENDED) {
            return;
        }
        auto sendLayer = RedPacketSendPacketLayerV77::create();
        if (sendLayer) {
...
//    void GameTableUIV77::myPacketHistoryButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
//        if (type != Widget::TouchEventType::ENDED) {
//            return;
//        }
//        if (_GameTableLogic) {
