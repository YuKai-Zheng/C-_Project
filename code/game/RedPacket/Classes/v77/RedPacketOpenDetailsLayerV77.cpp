//
//  RedPacketOpenDetailsLayerV77.cpp
//  MixProject_VIP
//
...
namespace RedPacket {
    ...
    RedPacketOpenDetailsLayerV77::RedPacketOpenDetailsLayerV77() {
    ...
    bool RedPacketOpenDetailsLayerV77::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void RedPacketOpenDetailsLayerV77::setSenderHead(UserInfoStruct *user) {
        if (!user) {
            if (m_userHead)
    ...
    void RedPacketOpenDetailsLayerV77::openAddToParent(Layer *parent, cocos2d::Vec2 pos) {
        if (parent) {
    ...
    void RedPacketOpenDetailsLayerV77::setPacketInfo(RedPacket::tag_c2s_RobRedPacketReq *info) {
        if (info) {
            m_packetInfo = *info;
            if (m_packetMoney) {
    ...
    //领取红包详情基础信息
    void RedPacketOpenDetailsLayerV77::setOpenPacketDetailInfo(RedPacket::tag_c2s_RedPacketRobLogReq *info) {
        if (info) {
    ...
    void RedPacketOpenDetailsLayerV77::setOpenPacketPlayerDetail(RobRedPacketUserInfo* userInfo) {
        if (m_detailsList) {
            //说明有玩家抢到红包，这个数据有效
            auto cell = RedPacketOpenDetailCellLayerV77::create();
            if (cell) {
    ...
    void RedPacketOpenDetailsLayerV77::setMyHistoryBtnCallBack(std::function<void ()> callBack) {
        m_myHistoryCallBack = callBack;
    }
    
    void RedPacketOpenDetailsLayerV77::resetData() {
        m_bg = nullptr;
        m_userHead = nullptr;
        m_userName = nullptr;
    ...
    void RedPacketOpenDetailsLayerV77::createUI() {
        ...
        if (m_bg) {
            ...
            if (m_closeButton) {
                ...
                m_closeButton->addTouchEventListener(CC_CALLBACK_2(RedPacketOpenDetailsLayerV77::backButtonCallBack, this));
            ...
            if (m_myHistoryButton) {
                ...
                m_myHistoryButton->addTouchEventListener(CC_CALLBACK_2(RedPacketOpenDetailsLayerV77::myHistoryButtonCallBack, this));
    ...
    void RedPacketOpenDetailsLayerV77::closeLayer() {
        this->removeFromParent();
    }
    
    void RedPacketOpenDetailsLayerV77::backButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
    ...
    void RedPacketOpenDetailsLayerV77::myHistoryButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        if (type != Widget::TouchEventType::ENDED) {
            return;
        }
