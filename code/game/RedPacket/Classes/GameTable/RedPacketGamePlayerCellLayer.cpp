//
//  RedPacketGamePlayerCellLayer.cpp
//  MixProject_VIP
//
...
namespace RedPacket {
    RedPacketGamePlayerCellLayer::RedPacketGamePlayerCellLayer() {
    ...
    bool RedPacketGamePlayerCellLayer::init() {
        if (!Button::init()) {
            return false;
        }
    ...
    void RedPacketGamePlayerCellLayer::resetData() {
        m_timeBg = nullptr;
...
//        m_redPacket = nullptr;
    ...
    void RedPacketGamePlayerCellLayer::createUI() {
        ...
        if (m_userHead) {
            ...
            if (m_userHeadButton) {
                ...
                m_userHeadButton->addTouchEventListener(CC_CALLBACK_2(RedPacketGamePlayerCellLayer::userHeadButtonCallBack, this));
        ...
        if (m_packetButton) {
            if (m_userHead) {
                ...
                m_packetButton->addTouchEventListener(CC_CALLBACK_2(RedPacketGamePlayerCellLayer::openPacketButtonCallBack, this));
    ...
    void RedPacketGamePlayerCellLayer::setHead(UserInfoStruct* user) {
        if (!user) {
    ...
    void RedPacketGamePlayerCellLayer::setPacketUIByMy(bool isMe) {
        m_isMe = isMe;
        if (!isMe) {
            if (m_userHead) {
...
//        }
    ...
    void RedPacketGamePlayerCellLayer::setParentUI(HNLayer* parent) {
        if (parent) {
            m_parent = parent;
        }
    }
    
    void RedPacketGamePlayerCellLayer::setOpenPacketCallBack(std::function<void(unsigned int)> callBack) {
        m_openPacketCallBack = callBack;
    }
    
    void RedPacketGamePlayerCellLayer::setpacketDetailCallBack(std::function<void (unsigned int)> callBack) {
        m_packetDetailCallBack = callBack;
    }
    
    void RedPacketGamePlayerCellLayer::setPacketData(tag_s2c_RedPacketListReq* object, UserInfoStruct *user) {
        if (object != nullptr) {
            m_redPacket = *object;
            if (user) {
...
//            }
    ...
    void RedPacketGamePlayerCellLayer::setMyPacketRobInfo(RedPacket::tag_c2s_RobRedPacketReq *packetInfo) {
        if (packetInfo) {
...
//    unsigned int RedPacketGamePlayerCellLayer::getPacketID() {
//        return m_packetID;
//    }
    
    void RedPacketGamePlayerCellLayer::setHasRobPacket(bool hasRob) {
        m_hasRobPacket = hasRob;
    }
    
    bool RedPacketGamePlayerCellLayer::getHasRobPacket() {
        return m_hasRobPacket;
    }
    
    void RedPacketGamePlayerCellLayer::setRequestMyInfoCallBack(std::function<void (unsigned int)> callBack) {
        m_requestMyInfo = callBack;
    }
    
    void RedPacketGamePlayerCellLayer::updateMyWalletMoney(std::string name, LLONG money) {
        RedPacketMyUserInfo* info = (RedPacketMyUserInfo*)this->getChildByName("MyInfo");
        if (info) {
            info->updateMyNameAndMoney(name, money);
        }
    }
    
    void RedPacketGamePlayerCellLayer::openPacketButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        if (type != Widget::TouchEventType::ENDED) {
            return;
        }
        
        //请求获取红包状态，如果红包已经被领取过，则显示红包详情；如果没有被领取过，显示拆红包界面。
        auto openLayer = RedPacket::RedPacketOpenPacketLayer::create();
...
//            openLayer->setShouldPlayAnimation(!m_hasRobPacket && !m_isMe)
    ...
    void RedPacketGamePlayerCellLayer::userHeadButtonCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        switch (type) {
            case Widget::TouchEventType::BEGAN: {
                auto info = RedPacketMyUserInfo::create();
                if (info) {
                    if (m_requestMyInfo) {
    ...
    void RedPacketGamePlayerCellLayer::updateLeftTime(float dt) {
