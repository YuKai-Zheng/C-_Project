...
//		LLONG				m_i64ShangXian;			//个人下注上限(即 最大每轮最大下注数不能超过这个值)  暗注的，但显示为明注的
	...
	//游戏下注数据包
	struct TMSG_GAME_NOTE_NTF
	{
		int			iDiZhu;			//本局底注
		int			iNoteCount[4];	//注数
		long long	i64DingZhu;		//顶注
        int			iOperateCount[2];	//第一个位置放当前局数，第二个位置为总局数
	...
	struct S_C_UserGenZhuResult
	{
		...
		LLONG		i64UserZongZhu[PLAY_COUNT];		//玩家的总下注
    ...
    //用户金币不够做提醒
    struct S_C_UserLackMoneyTip
    {
        BYTE	byDeskStation;				//玩家IDDEX
    };
	/*-----------------------------------------------------------------------------*/
	//用户比牌
	...
	enum
	{
		...
		S_C_GAME_NOTE_NTF = 90,				//游戏下注
        S2C_TIP_EXIT_MSG = 91,          //金币不足退出游戏
        S_C_LACK_MONEY_TIP = 92,				//金币不足提示
