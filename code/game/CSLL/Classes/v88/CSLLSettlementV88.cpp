...
namespace CSLL {
	...
	CSLLSettlementV88::CSLLSettlementV88() {
    }
    
	CSLLSettlementV88::~CSLLSettlementV88() {
    }
    
	bool CSLLSettlementV88::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void CSLLSettlementV88::showSettleLayerAndUpdateData(struct_s2c_game_begin msg) {
        int count=0;
        m_totalRate = 0;
        scoreCellVec.clear();
        for (int i = 0; i < LINE_COUNT; i++) {
            if (msg.nOpenLineLotteryType[i] != -1) {
                CSLLScorePanelCellV88* newCell = CSLLScorePanelCellV88::create();
    ...
    void CSLLSettlementV88::hideSettleLayer() {
    ...
    void CSLLSettlementV88::initSettleLayer() {
        m_audioEffectID = 0;
        m_root = nullptr;
        m_action = nullptr;
        scoreCellVec.clear();
