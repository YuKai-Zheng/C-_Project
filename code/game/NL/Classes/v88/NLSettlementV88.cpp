...
namespace NL {
	...
	NLSettlementV88::NLSettlementV88() {
    }
    
	NLSettlementV88::~NLSettlementV88() {
    }
    
	bool NLSettlementV88::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void NLSettlementV88::showSettleLayerAndUpdateData(struct_s2c_game_begin msg) {
        int count=0;
        m_totalRate = 0;
        scoreCellVec.clear();
        for (int i = 0; i < LINE_COUNT; i++) {
            if (msg.nOpenLineLotteryType[i] != -1) {
                NLScorePanelCellV88* newCell = NLScorePanelCellV88::create();
    ...
    void NLSettlementV88::hideSettleLayer() {
    ...
    void NLSettlementV88::initSettleLayer() {
        m_audioEffectID = 0;
        m_root = nullptr;
        m_action = nullptr;
        scoreCellVec.clear();
