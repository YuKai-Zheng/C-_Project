...
namespace Tiger {
	...
	TigerSettlementV88::TigerSettlementV88() {
    }
    
	TigerSettlementV88::~TigerSettlementV88() {
    }
    
	bool TigerSettlementV88::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void TigerSettlementV88::showSettleLayerAndUpdateData(struct_s2c_game_begin msg) {
        int count=0;
        m_totalRate = 0;
        scoreCellVec.clear();
        for (int i = 0; i < LINE_COUNT; i++) {
            if (msg.nOpenLineLotteryType[i] != -1) {
                TigerScorePanelCellV88* newCell = TigerScorePanelCellV88::create();
    ...
    void TigerSettlementV88::hideSettleLayer() {
    ...
    void TigerSettlementV88::initSettleLayer() {
        m_audioEffectID = 0;
        m_root = nullptr;
        m_action = nullptr;
        scoreCellVec.clear();
