...
namespace Tiger {
    ...
    const static char* Tiger_BGM = "Games/Tiger/music/medusaBg.mp3";
    ...
    const static char* kCsbRoot = "Games/Tiger/ui.csb";
    const static char* kIconTypeTpl = "Games/Tiger/type/%02d.png";
    const static char* kHelpFile = "Games/Tiger/help/tiger.png";
        ...
        if (middle) {
            ...
            if (bg) {
                ...
                if (jackpotText)
                {
					...
					_root.jackpot->setTextHorizontalAlignment(TextHAlignment::CENTER);
					_root.jackpot->setTextVerticalAlignment(TextVAlignment::CENTER);

					float fY = 10;
					float fX = 0;
					_root.jackpot->setPositionY(_root.jackpot->getPositionY() + fY);
                }
                
                Text* addLineText = dynamic_cast<Text*>(bg->getChildByName("txtLine"));
                if (addLineText) {
                    _root.addLineCount = addLineText;
					_root.addLineCount->setTextHorizontalAlignment(TextHAlignment::CENTER);
					_root.addLineCount->setTextVerticalAlignment(TextVAlignment::CENTER);

					float fY = -2;
					float fX = -60;
					_root.addLineCount->setPositionX(_root.addLineCount->getPositionX() + fX);
					_root.addLineCount->setPositionY(_root.addLineCount->getPositionY() + fY);
                }
                ...
                if (start) {
                    ...
                    _root.startBtn ->addTouchEventListener(CC_CALLBACK_2(GameTableUIV88::startBtnCallBack, this));
                }
...
//                Button* maxLine = dynamic_cast<Button*>(bg->getChildByName("Button_max_line"));
//                if (maxLine) {
...
//                
//                // 九线
//                auto nineLine = dynamic_cast<Button*>(bg->getChildByName("Button_nine_line"));
//                if (nineLine)
//                {
...
//                }
//                
//                Button* subLine = dynamic_cast<Button*>(bg->getChildByName("btnSubLine"));
//                if (subLine) {
//                    _root.subLine = subLine;
//                    subLine->addTouchEventListener(CC_CALLBACK_2(GameTableUIV88::subLineBtnCallBack, this));
//                }
    ...
    void GameTableUIV88::createGameSettlementLayer() {
        ...
        TigerSettlementV88* settle = TigerSettlementV88::create();
        if (settle) {
	...
	void GameTableUIV88::createToolBarUI() {
		...
		if (toolbar) {
            ...
            toolbar->addImageAndCallBack(TOOL_BAR_HELP, [=]() {
                ...
                //helpLayer->setBgImage("Games/Tiger/help/bg.png");
                ...
                //helpLayer->setPosition(WIN_SIZE);//Vec2(WIN_SIZE.width/2,WIN_SIZE.height)
...
//        i64MyMoney = 0;
    ...
    void GameTableUIV88::setTurnUnitCount(vector<int> &vTurnUnitCount, vector<int> vCurBeginIcon, vector<int> vCurEndIcon, int nRoundMinNum) {
        for (int i = 0; i < LOTTERY_COUNT_Tiger; i++) {
            //整圈数
    ...
    void GameTableUIV88::transformOpenLotteryType() {
        ...
        for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
            for (int j = 0; j < 3; j++) {
                int index = (3-j-1)*5+i;
                if (index >=0 && index < LOTTERY_COUNT_Tiger) {
