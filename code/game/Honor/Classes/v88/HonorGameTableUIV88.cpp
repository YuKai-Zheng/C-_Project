...
#include "HonorFinalModeReward.hpp"
...
namespace Honor {
    ...
    const static char* Tiger_BGM = "Games/31000513/music/bg.mp3";
    const static char* Honor_Plane_BGM = "Games/31000513/music/plane.wav";
    const static char* Honor_Gun_BGM = "Games/31000513/music/boom.wav";
	const static char* Honor_Fire = "Games/31000513/music/fire.mp3";
    ...
    const static char* kCsbRoot = "Games/31000513/Node_root.csb";
    const static char* kIconTypeTpl = "Games/31000513/type/%d.png";
    const static char* kHelpFile = "Games/31000513/help/help.png";
    const static char* kJackpotUI = "common/animation/jackPot/Jackpot.csb";

	const static char* jsonPath = "Games/31000513/spine/direnjie_xulifashe.json";
	const static char* atlasPath = "Games/31000513/spine/direnjie_xulifashe.atlas";
	const static char* jsonPathGame = "Games/31000513/spine/luban_fashe.json";
	const static char* atlasPathGame = "Games/31000513/spine/luban_fashe.atlas";
	const static char* jsonPathGameFire = "Games/31000513/spine/wzry_huoyan_fx.json";
	const static char* atlasPathGameFire = "Games/31000513/spine/wzry_huoyan_fx.atlas";

	const static char* bombCsbPath = "Games/31000513/Node_bomb.csb";
	...
	GameTableUIV88::GameTableUIV88()
		...
		, m_skeletonGame(nullptr)
		, m_skeletonGameFire(nullptr)
...
//        }
        ...
        if (middle) {
            ...
            if (bg) {
				...
				_root.myID = dynamic_cast<Text*>(bg->getChildByName("Image_53")->getChildByName("txtid"));
...
//                // 九线
    ...
    void GameTableUIV88::loadTurn() {
        ...
        for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
            ...
            vIcon.clear();
            for (int j = 0; j < LOTTERY_TYPE_COUNT_Honor; j++) {
       ...
       // auto pos = m_vPanelTurnArea.at(0)->convertToWorldSpace(m_vTurnIcon.at(0).at(0)->getPosition());
        ...
        _root.smallMirror = ImageView::create("Games/31000513/ui/smallMirror.png");
        ...
        if (bg) {
            ...
            auto posEx = _root.panelBg->convertToNodeSpace(pos);
            _root.smallMirror->setPosition(posEx);
            _root.panelBg->addChild(_root.smallMirror);
        }
        
        _root.bigMirror = ImageView::create("Games/31000513/ui/bigMirror.png");
        ...
        if (bg) {
            ...
            auto posEx = _root.panelBg->convertToNodeSpace(pos);
            _root.bigMirror->setPosition(posEx);
            _root.panelBg->addChild(_root.bigMirror);
            _root.bigMirror->setVisible(false);
        }
        
        _root.bigKillModeBg = ImageView::create("Games/31000513/ui/bigKill.png");
            ...
            for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
                for (int j = 0; j < 4; j++) {
                    ...
                    if (index == idx) {
                        ...
                        auto posEx = _root.panelBg->convertToNodeSpace(pos);
                        _root.smallMirror->runAction(Sequence::create(MoveTo::create(4.5f,posEx),
							CallFunc::create([this](){
                            if (m_bAirDrop) {
                                HNAudioEngine::getInstance()->playEffect(Honor_Gun_BGM);
                                //ImageView* boom = ImageView::create("Games/31000513/ui/tankong.png");
                                //if (boom) {
								...
								auto nodeName = "bombNode";
								auto node = this->getChildByName(nodeName);
								if (nullptr == node)
								{
									node = CSLoader::createNode(bombCsbPath);
									node->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
									node->setPosition(_root.smallMirror->getContentSize() / 2);
									_root.smallMirror->addChild(node);
								}
								node->stopAllActions();

								auto action = CSLoader::createTimeline(bombCsbPath);
   ...
   void GameTableUIV88::BigMirrorMove(int idx)
            ...
            for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
                for (int j = 0; j < 4; j++) {
                    ...
                    if (index == idx) {
                        auto pos = m_vPanelTurnArea.at(i)->convertToWorldSpace(m_vTurnIconPos.at(j));
                        auto posEx = _root.panelBg->convertToNodeSpace(pos);
                        _root.bigMirror->runAction(Sequence::create(MoveTo::create(4.5f,posEx),CallFunc::create([this](){
                        }),NULL));

						if (m_skeletonGameFire)
						{
							Vec2 vPos(posEx.x-200, posEx.y-460);
							m_skeletonGameFire->runAction(Sequence::create(
								DelayTime::create(2.0f),
								CallFunc::create([this]() {
									if(m_skeletonGame) m_skeletonGame->setAnimation(0, "luban_fashe", false);
								}), MoveTo::create(2.0f, vPos), FadeIn::create(1.0f), Blink::create(3.0f, 3),
								CallFunc::create([this]() {
									HNAudioEngine::getInstance()->playEffect(Honor_Fire);
								}), NULL));
						}
                    }
                }
    ...
    void GameTableUIV88::createGameSettlementLayer() {
        ...
        HonorSettlementV88* settle = HonorSettlementV88::create();
        if (settle) {
    ...
    void GameTableUIV88::onEnter() {
		...
		char ch[12] = { 0 };
		sprintf(ch, "%d", _GameTableLogic->getUserBySeatNo(_GameTableLogic->getMySeatNo())->dwUserID);
		std::string str(GBKToUtf8(string(ch).c_str()));
		_root.myID->setString(str);
		//////////////////////////////
		/////////////////////
		_GameTableLogic->sendGameInfo();

		Node* skeletonParent = nullptr;
		if (_root.middleNode) {
			skeletonParent = _root.middleNode->getChildByName("Image_bg");
		}

		m_skeletonNode = spine::SkeletonAnimation::createWithJsonFile(jsonPath, atlasPath);
		if (m_skeletonNode) {
			m_skeletonNode->setPosition(800, -100);
			m_skeletonNode->setAnimation(0, "direnjie_xuli", false);
			m_skeletonNode->setCompleteListener([=](int trackIndex, int loopCount) {
				m_skeletonNode->setAnimation(0, "direnjie_xuli", false);
			});
			if (skeletonParent) skeletonParent->addChild(m_skeletonNode);
		}
	}
...
//    }
	...
	void GameTableUIV88::startBtnCallBack(cocos2d::Ref *pSender, Widget::TouchEventType type) {
        ...
        if (m_bFinalMode) { //大逃杀模式
			if (m_skeletonGameFire) m_skeletonGameFire->setOpacity(0);
...
//        msg->nLineConstitute;
    ...
    void GameTableUIV88::airDropFunc()
    {
        ...
        ImageView * plane = ImageView::create("Games/31000513/ui/plane.png");
        if (plane) {
            ...
            HNAudioEngine::getInstance()->playEffect(Honor_Plane_BGM);
            this->runAction(Sequence::create(CallFunc::create([plane](){
                ...
                if (vecPos.size() > 0) {
                    for (int n = 0; n < vecPos.size(); n++) {
                        auto img = ImageView::create("Games/31000513/ui/shan.png");
                        if (img) {
                                ...
                                if (imgVec.size() > 0) {
                                    for (int j = 0; j < imgVec.size(); j++) {
                                        ...
                                        if (img) {
                                            ...
                                            img->setScale(1.0);
                                            img->loadTexture("Games/31000513/type/9.png");
                                        }
                                    }
                                }
	...
	void GameTableUIV88::setViewVisible(bool bSmallGame)
	{
		_root.smallMirror->setVisible(!bSmallGame);
		_root.bigMirror->setVisible(bSmallGame);
		_root.bigMirror->setOpacity(0);
		bigKillModeShow(bSmallGame);

		if(m_skeletonNode) m_skeletonNode->setVisible(!bSmallGame);
		if (m_skeletonGame) m_skeletonGame->setVisible(bSmallGame);
		if (m_skeletonGameFire) m_skeletonGameFire->setOpacity(0);
	}
...
//        i64MyMoney = 0;
    ...
    void GameTableUIV88::turnUpdate(float dt) {
                ...
                if (m_nHasTurnUnitCount <= m_vTurnUnitCount.at(i)) {
                    ...
                    if (img) {
                        ...
                        img->runAction(Sequence::create(MoveBy::create(0.1f, Vec2(0, -ICON_HEIGHT)), CallFunc::create([=]() {
                            ...
                            int nNewTag = (nOldTag - 1) < 0 ? (LOTTERY_TYPE_COUNT_Honor - 1) : (nOldTag - 1);
                            m_vTurnIcon.at(i).at(j)->setTag(nNewTag);
                            
                            //一轮结束
                            if (j == LOTTERY_TYPE_COUNT_Honor - 1) {
                                for (int k = 0; k < LOTTERY_TYPE_COUNT_Honor; k++) {
                    ...
                    if (m_nHasTurnUnitCount == m_vTurnUnitCount.at(i))
                    {
                        ...
                        for (auto ik = 0; ik < LOTTERY_TYPE_COUNT_Honor; ++ik)
                        {
        ...
        if (m_nHasTurnUnitCount == m_vTurnUnitCount.at(LOTTERY_COLUMN_COUNT - 1))// && j == (LOTTERY_TYPE_COUNT_Tiger - 1))
        {
            ...
            if (m_bFirstMod) {
                ...
                {   float delayTime = m_bFinalMode ? 1.0 : 0;
                    runAction(Sequence::create(
						CallFunc::create([this]() {
						if (m_bFinalMode) m_skeletonNode->setAnimation(0, "direnjie_fashe", false);
						}),
							DelayTime::create(delayTime*2),
							CallFunc::create([this](){
                            if (m_bFinalMode) {
                                for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
                                    for (int j = 0; j < 4; j++) {
                                        ...
                                        if (index == nGunsignPositionSmall) {
                                            ...
                                            if (result) {
                                                ...
                                                //}
												auto node = CSLoader::createNode(bombCsbPath);
												node->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
												node->setPosition(result->getContentSize() / 2);
            ...
            else if (m_bFinishFinalMode) //结束大逃杀模式转换为普通模式
            {
                ...
                auto layer = HonorFinalRewardLayer::create();
                if (layer) {
    ...
    void GameTableUIV88::bigKillModeFunc()
    {
        if (m_bFinalMode) {
            ...
            ImageView* tip = ImageView::create("Games/31000513/ui/tip.png");
            if (tip) {
			...
			if (nullptr == m_skeletonGame) {
				m_skeletonGame = spine::SkeletonAnimation::createWithJsonFile(jsonPathGame, atlasPathGame);
				Vec2 vPos(600, -100);
				m_skeletonGame->setPosition(vPos);
				m_skeletonGame->setAnimation(0, "luban_fashe", false);

				if (_root.bigKillModeBg) _root.bigKillModeBg->addChild(m_skeletonGame);
			}
			else
			{
				m_skeletonGame->setAnimation(0, "luban_fashe", false);
			...
			if (nullptr == m_skeletonGameFire) {
				m_skeletonGameFire = spine::SkeletonAnimation::createWithJsonFile(jsonPathGameFire, atlasPathGameFire);
				Vec2 vPos(300, -140);
				m_skeletonGameFire->setPosition(vPos);
				m_skeletonGameFire->setAnimation(0, "wzry_huoyan_fx", true);
				m_skeletonGameFire->setOpacity(0);

				if (_root.panelBg) _root.panelBg->addChild(m_skeletonGameFire);
			}
        }
    ...
    void GameTableUIV88::setTurnIcomPosition(vector<Vec2> &vIconPos, Size parentSize,int index) {
        ...
        for (int i = 0; i < LOTTERY_TYPE_COUNT_Honor; i++) {
    ...
    void GameTableUIV88::setTurnUnitCount(vector<int> &vTurnUnitCount, vector<int> vCurBeginIcon, vector<int> vCurEndIcon, int nRoundMinNum) {
        for (int i = 0; i < LOTTERY_COUNT_Honor; i++) {
            //整圈数
            int nCount = (nRoundMinNum + i)*LOTTERY_TYPE_COUNT_Honor;
            //开的那圈
            nCount += LOTTERY_TYPE_COUNT_Honor - vCurBeginIcon.at(i) + vCurEndIcon.at(i);
    ...
    void GameTableUIV88::transformOpenLotteryType() {
        ...
        for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
            for (int j = 0; j < 4; j++) {
                int index = (4-j-1)*5+i;
                if (index >=0 && index < LOTTERY_COUNT_Honor) {
    ...
    void GameTableUIV88::randomColumnDatasAndCalcTurnCount() {
            ...
            for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
                ...
                for (int j = 0; j < LOTTERY_TYPE_COUNT_Honor; j++) {
                    ...
                    if (img) {
                        ...
                        if (imgTag >= LOTTERY_TYPE_COUNT_Honor - 4) {
                            sprintf(str, kIconTypeTpl, nOpenLotteryTypeColRow[i][index++]);
                        }
                        else {
                            sprintf(str, kIconTypeTpl, rand() % LOTTERY_TYPE_COUNT_Honor);
                        }
                        img->loadTexture(str);
                    }
                }
        ...
        m_vTurnUnitCount.clear();
        for (int i = 0; i < LOTTERY_COLUMN_COUNT; i++) {
            int nCount = (2 + i) * LOTTERY_TYPE_COUNT_Honor;
            nCount += (LOTTERY_TYPE_COUNT_Honor - 4);
