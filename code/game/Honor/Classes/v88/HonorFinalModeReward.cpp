...
HonorFinalRewardLayer::HonorFinalRewardLayer()
: topTen(nullptr)
, numberOne(nullptr)
, rank1Text(nullptr)
, rank2Text(nullptr)
...
void HonorFinalRewardLayer::showData(int rank,int killPeoPleNum,long long getMoneyNum,Node* parent) {
    if (rank) {
...
bool HonorFinalRewardLayer::init() {
    if (!HNLayer::init())
        return false;
    ...
    Node* middle = CSLoader::createNode("Games/31000513/RewardNode.csb");
    if (middle) {
        ...
        if (imgBg) {
            ...
            if (buttonOK) {
                buttonOK->addTouchEventListener(CC_CALLBACK_2(HonorFinalRewardLayer::setOkayCallBack, this));
            }
        }
    }
...
void HonorFinalRewardLayer::setOkayCallBack(Ref* pSender, Widget::TouchEventType type) {
