...
BYTE GameLogic::GetCardValue(BYTE cbCardData) {
	return cbCardData & LOGIC_MASK_VALUE;
}

BYTE GameLogic::GetCardColor(BYTE cbCardData) {
...
BYTE GameLogic::GetCardPip(BYTE cbCardData)
{
...
BYTE GameLogic::GetCardListPip(const BYTE cbCardData[], BYTE cbCardCount)
{
	BYTE cbPipCount=0;
...
int GameLogic::GetPoint(BYTE wCard)
{
	if (wCard == 0x00)
		return 0;
...
int GameLogic::CountPoint(BYTE wCardList[],int iCardCount)
{
	int point = 0;
...
int GameLogic::CanSumIn(BYTE wCardList[],int iCardCount,int iSelectNum)
{
...
///@param: int  iCardCount  [in] 牌张数
///@return: int [out]  最大牌
int GameLogic::GetMaxCard(BYTE wCardList[],int iCardCount)
{
	int temp = 0;
	BYTE card = 0;
	for (int i = 0; i < iCardCount; i++)
...
int GameLogic::CountKing(BYTE wCardList[],int iCardCount)
{
	int temp=0;
	for (int i=0; i<iCardCount; i++)
	{
...
bool GameLogic::IsBullBull(BYTE wCardList[],int iCardCount)
{
	int total=CountPoint(wCardList,iCardCount);
...
int GameLogic::IsHaveNote(BYTE wCardList[],int iCardCount)
{
	int Note = CanSumIn(wCardList, iCardCount, 3);
	return (Note % 10);
}

int GameLogic::GetMaxPoint(BYTE wCardList[],int iCardCount,BYTE wBullCard[])
{
	int iMaxPoint = 0;
	int iTempPoint= 0;
	for (int i=0; i<4; i++)
...
///@param: int  iCardCount  [in] 牌张数
///@return: int [out]  返回牌型
int GameLogic::GetShape(BYTE wCardList[],int iCardCount)
{
...
bool GameLogic::GetBull(BYTE wCardList[],int iCardCount,BYTE wBullCard[])
{
//		int total = CountPoint(wCardList,iCardCount);
	for (int i=0; i<3; i++)
	{
