...
#define ONE_PIECE_BGM					"Games/31000505/music/slotBgm.mp3"
#define ONE_PIECE_START_EFFECT			"Games/31000505/music/start.mp3"
#define ONE_PIECE_TURN_EFFECT			"Games/31000505/music/turn.mp3"
#define ONE_PIECE_TURN_DOWN_EFFECT		"Games/31000505/music/turnDown.mp3"
#define ONE_PIECE_SCORE_WIN_EFFECT		"Games/31000505/music/scoreWin.mp3"
#define ONE_PIECE_SCORE_LOSE_EFFECT		"Games/31000505/music/scoreLose.mp3"
#define ONE_PIECE_WIN_EFFECT			"Games/31000505/music/win.mp3"
#define ONE_PIECE_LOSE_EFFECT			"Games/31000505/music/lose.mp3"

#define ONE_PIECE_SLOT_DOUBLE_WIN_ANI		"v88/animation/slots/onepieceslots/onepieceslots_win/onepieceslots_win.csb"
#define ONE_PIECE_SLOT_DOUBLE_LOSE_ANI		"v88/animation/slots/onepieceslots/onepieceslots_lose/onepieceslots_lose.csb"

#define ONE_PIECE_SLOT_DOUBLE_RESULT_NUM_ANI	"v88/animation/slots/onepieceslots/onepieceslots_sizetip/onepieceslots_sizetip.csb"

const static char* kIconTypeTpl = "Games/31000505/icon%d.png";
...
void GameTableUI::onEnter() {
	...
	HNAudioEngine::getInstance()->playBackgroundMusic(ONE_PIECE_BGM, true);

	m_csb = CSLoader::createNode("csb/v66/OnePieceSlot/OnePieceSlotUI.csb");
...
void GameTableUI::onGamePoolMoney(struct_s2c_pool* msg)
{
	if (msg->i64PoolMoney > 0)
	{
		m_textPoolMoney->setString(to_string(msg->i64PoolMoney));
	}
}
...
void GameTableUI::onGameStart(struct_s2c_startGame* msg)
{
	...
	m_nTurnMusicID = HNAudioEngine::getInstance()->playEffect(ONE_PIECE_TURN_EFFECT, true);
}
...
void GameTableUI::onGameFinish(struct_s2c_caiFinish* msg)
{
    ...
    if (coin) {
        coin->openParent(this, Vec2(WIN_SIZE.width*0.4f, WIN_SIZE.height*0.5f-200), 100);
        coin->updateCoin(msg->i64WinMoney);
    }
	//音效
	if (msg->i64WinMoney > 0)
	{
		HNAudioEngine::getInstance()->playEffect(ONE_PIECE_WIN_EFFECT);
	}
	else if (msg->i64WinMoney < 0)
	{
		HNAudioEngine::getInstance()->playEffect(ONE_PIECE_LOSE_EFFECT);
	}
...
void GameTableUI::autoBtnCallback(Ref* pSender, Widget::TouchEventType type)
{
	...
	if (_GameTableLogic->_gameStatus == GS_FREE && m_bAuto)
	{
		...
		if (PlatformLogic()->loginResult.i64Money < i64NeedMoney)
		{
...
void GameTableUI::startBtnCallback(Ref* pSender, Widget::TouchEventType type)
{
	...
	HNAudioEngine::getInstance()->playEffect(ONE_PIECE_START_EFFECT);
	...
	//调拉拔动画
	m_barAction->play("onepieceslots_bar", false);
}
...
{
	m_btnAuto = dynamic_cast<Button*>(m_csb->getChildByName("Button_auto"));
	...
	m_btnCancel = dynamic_cast<Button*>(m_csb->getChildByName("Button_cancel"));
	...
	m_btnBet = dynamic_cast<Button*>(m_csb->getChildByName("Button_bet"));
	...
	m_btnBetMax = dynamic_cast<Button*>(m_csb->getChildByName("Button_betMax"));
	...
	m_btnStart = dynamic_cast<Button*>(m_csb->getChildByName("Button_turn"));
	...
	m_btnScore = dynamic_cast<Button*>(m_csb->getChildByName("Button_score"));
    m_btnScore->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	m_btnScore->setTouchEnabled(true);
	m_btnScore->setVisible(true);
	m_btnScore->setTag(0);
	m_btnScore->addTouchEventListener(CC_CALLBACK_2(GameTableUI::doubleBtnCallback, this));

	m_btnDouble = dynamic_cast<Button*>(m_csb->getChildByName("Button_double"));
    m_btnDouble->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	m_btnDouble->setTouchEnabled(true);
	...
	//下分比倍外发光
	m_imageScoreFrame = ImageView::create("Games/31000505/5_1.png");
    m_imageScoreFrame->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
    m_imageScoreFrame->setVisible(false);
	m_imageScoreFrame->setPosition(m_btnScore->getPosition());
	m_csb->addChild(m_imageScoreFrame);

	m_imageDoubleFrame = ImageView::create("Games/31000505/6_1.png");
    m_imageDoubleFrame->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
    m_imageDoubleFrame->setVisible(false);
	m_imageDoubleFrame->setPosition(m_btnDouble->getPosition());
	m_csb->addChild(m_imageDoubleFrame);

	//拉拔
	m_btnBar = dynamic_cast<Button*>(m_csb->getChildByName("Button_bar"));
	m_btnBar->addTouchEventListener(CC_CALLBACK_2(GameTableUI::startBtnCallback, this));
}

void GameTableUI::loadText()
{
	m_textBetUnit = dynamic_cast<Text*>(m_csb->getChildByName("Text_betBase"));
	m_textBetUnit->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	m_textBetUnit->setString("");

	m_textBetMul = dynamic_cast<Text*>(m_csb->getChildByName("Text_betMul"));
	m_textBetMul->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	m_nCurBetMul = 1;
	m_textBetMul->setString(to_string(m_nCurBetMul));

	m_textPoolMoney = dynamic_cast<Text*>(m_csb->getChildByName("Text_poolMoney"));
	m_textPoolMoney->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	m_textPoolMoney->setString("0");

	m_textMyMoney = dynamic_cast<Text*>(m_csb->getChildByName("Text_money"));
	...
	m_textScoreMoney = dynamic_cast<Text*>(m_csb->getChildByName("Text_score"));
...
void GameTableUI::loadTurn()
{
	...
	{
		//转的容器
		Layout* layout = dynamic_cast<Layout*>(m_csb->getChildByName(StringUtils::format("Panel_turnArea%d", i)));
		layout->setClippingEnabled(true);
		m_vPanelTurnArea.push_back(layout);

		//开奖闪烁
		ImageView* imageTurnBlink = dynamic_cast<ImageView*>(m_csb->getChildByName(StringUtils::format("Image_turnBlink%d", i)));
	...
	//拉bar动画
	Node* nodeBar = dynamic_cast<Node*>(m_csb->getChildByName("Node_bar"));

	auto barCsb = CSLoader::createNode("v88/animation/slots/onepieceslots/onepieceslots_bar.csb");
	barCsb->setPosition(Vec2(-17, 10));
	nodeBar->addChild(barCsb);
	m_barAction = CSLoader::createTimeline("v88/animation/slots/onepieceslots/onepieceslots_bar.csb");
	...
	for (int i = 0; i < LOTTERY_COUNT; i++)
	{
		auto blinkCsb = CSLoader::createNode("v88/animation/slots/onepieceslots/onepieceslots_guangzao.csb");
		...
		m_csb->addChild(blinkCsb);
		m_vTurnBlinkCsb.push_back(blinkCsb);

		auto blinkAction = CSLoader::createTimeline("v88/animation/slots/onepieceslots/onepieceslots_guangzao.csb");
	...
	m_imageOpenTypeBlink = ImageView::create("Games/31000505/guang2.png");
	m_imageOpenTypeBlink->setVisible(false);
...
{
	m_panelDouble = dynamic_cast<Layout*>(m_csb->getChildByName("Panel_double"));
	m_panelDouble->setVisible(false);
	...
	m_caiWinCsb = CSLoader::createNode(ONE_PIECE_SLOT_DOUBLE_WIN_ANI);
	m_caiWinCsb->setPosition(pos);
	m_caiWinCsb->setVisible(false);
	m_panelDouble->addChild(m_caiWinCsb);
	m_caiWinAction = CSLoader::createTimeline(ONE_PIECE_SLOT_DOUBLE_WIN_ANI);
	...
	m_caiLoseCsb = CSLoader::createNode(ONE_PIECE_SLOT_DOUBLE_LOSE_ANI);
	m_caiLoseCsb->setPosition(pos);
	m_caiLoseCsb->setVisible(false);
	m_panelDouble->addChild(m_caiLoseCsb);
	m_caiLoseAction = CSLoader::createTimeline(ONE_PIECE_SLOT_DOUBLE_LOSE_ANI);
	...
	m_caiResultCsb0 = CSLoader::createNode(ONE_PIECE_SLOT_DOUBLE_RESULT_NUM_ANI);
	m_caiResultCsb0->setPosition(m_btnSmall->getPosition());
	m_caiResultCsb0->setVisible(false);
	m_panelDouble->addChild(m_caiResultCsb0);
	m_caiResultAction0 = CSLoader::createTimeline(ONE_PIECE_SLOT_DOUBLE_RESULT_NUM_ANI);
	...
	m_caiResultCsb1 = CSLoader::createNode(ONE_PIECE_SLOT_DOUBLE_RESULT_NUM_ANI);
	m_caiResultCsb1->setPosition(m_btnBig->getPosition());
	m_caiResultCsb1->setVisible(false);
	m_panelDouble->addChild(m_caiResultCsb1);
	m_caiResultAction1 = CSLoader::createTimeline(ONE_PIECE_SLOT_DOUBLE_RESULT_NUM_ANI);
...
void GameTableUI::createToolBarUI() {
	...
	if (toolbar) {
		...
		toolbar->addImageAndCallBack(TOOL_BAR_SETTING, [=]() {
			...
			gameSet->setBgPath(ONE_PIECE_BGM);
...
void GameTableUI::turnUpdate(float dt)
{
		...
		for (int j = 0; j < LOTTERY_TYPE_COUNT; j++)
		{
			if (m_nHasTurnUnitCount <= m_vTurnUnitCount.at(i))
			{
				m_vTurnIcon.at(i).at(j)->runAction(Sequence::create(
					MoveBy::create(m_fTurnUnitTime, Vec2(0, -ICON_HEIGHT)), CallFunc::create([=](){
					...
					if (m_nHasTurnUnitCount == m_vTurnUnitCount.at(i))
					{
						HNAudioEngine::getInstance()->playEffect(ONE_PIECE_TURN_DOWN_EFFECT);
					}

				}), nullptr));
                
                if (m_nHasTurnUnitCount == m_vTurnUnitCount.at(i) && j == (LOTTERY_TYPE_COUNT - 1))
                {
                    char str[128] = {0};
    ...
    //最后格子停下来后
    if (m_nHasTurnUnitCount == m_vTurnUnitCount.at(LOTTERY_COUNT - 1))// && j == (LOTTERY_TYPE_COUNT_CSLL - 1))
    {
...
void GameTableUI::setTurnUnitCount(vector<int> &vTurnUnitCount, vector<int> vCurBeginIcon, vector<int> vCurEndIcon, int nRoundMinNum)
{
	...
	{
		//整圈数
		int nCount = (nRoundMinNum + i)*LOTTERY_TYPE_COUNT;
		//开的那圈
		nCount += LOTTERY_TYPE_COUNT - vCurBeginIcon.at(i) + vCurEndIcon.at(i);
...
void GameTableUI::checkTurnIsSuccess()
{
	...
	if (m_nOpenLineType >= 0)
	{
		...
		for (int i = 0; i < LOTTERY_COUNT; i++)
		{
			...
			m_vTurnBlinkAction.at(i)->play("onepieceslots_guangzao", false);
		}
...
void GameTableUI::showCaiResultEffect(bool bSuccess, int nPoint)
{
	runAction(Sequence::create(DelayTime::create(0.5f), CallFunc::create([=](){
		...
		if (bSuccess)
		{
			HNAudioEngine::getInstance()->playEffect(ONE_PIECE_SCORE_WIN_EFFECT);

			m_caiWinCsb->setVisible(true);
			m_caiWinAction->play("onepieceslots_win", false);
		}
		else
		{
			HNAudioEngine::getInstance()->playEffect(ONE_PIECE_SCORE_LOSE_EFFECT);

			m_caiLoseCsb->setVisible(true);
			m_caiLoseAction->play("onepieceslots_lose", false);
		}

		if (nPoint >= 1 && nPoint <= 5)
		{
			m_caiResultCsb0->setVisible(true);
			m_caiResultAction0->play("onepieceslots_sizetip", false);
		}

		if (nPoint >= 5 && nPoint <= 9)
		{
			m_caiResultCsb1->setVisible(true);
			m_caiResultAction1->play("onepieceslots_sizetip", false);
		}
