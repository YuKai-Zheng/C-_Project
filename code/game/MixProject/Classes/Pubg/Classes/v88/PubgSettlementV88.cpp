...
namespace Pubg {
	...
	PubgSettlementV88::PubgSettlementV88() {
    }
    
	PubgSettlementV88::~PubgSettlementV88() {
    }
    
	bool PubgSettlementV88::init() {
        if (!Layer::init()) {
            return false;
        }
    ...
    void PubgSettlementV88::showSettleLayerAndUpdateData(struct_s2c_game_begin msg) {
        int count=0;
        m_totalRate = 0;
        scoreCellVec.clear();
        for (int i = 0; i < LINE_COUNT; i++) {
            if (msg.nOpenLineLotteryType[i] != -1) {
                PubgScorePanelCellV88* newCell = PubgScorePanelCellV88::create();
    ...
    void PubgSettlementV88::hideSettleLayer() {
    ...
    void PubgSettlementV88::initSettleLayer() {
        m_audioEffectID = 0;
        m_root = nullptr;
        m_action = nullptr;
        scoreCellVec.clear();
